...
template< class dataStruct> int my_getopt_long_only(int argc, char ** argv, dataStruct *opts){
	char *name = new char[PARM_LENGЕTH];
	char *value = new char[PARM_LENGЕTH];
	bool nameInit = false;
	...
	//Разделить строку, если там есть равно
		...
		if(eqPos!=NULL){
			...
			if(sscanf_s(namePart, "--%s", name, PARM_LENGЕTH))
				nameInit = true;
			if (sscanf_s(++eqPos, "%s", value, PARM_LENGЕTH))
				valueInit = true;
		}
		//читаем отдельно название атрибута
		else if (!nameInit){
			if(sscanf_s(argv[i], "--%s", name, PARM_LENGЕTH))
					nameInit=true;
		}
		// читаем значение атрибута
		else if (!valueInit){
			if (sscanf_s(argv[i], "%s", value, PARM_LENGЕTH)){
				valueInit = true;
			}
		}
